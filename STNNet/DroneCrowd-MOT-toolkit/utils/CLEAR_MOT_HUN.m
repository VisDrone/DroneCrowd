...
Fgt = max(gtMat(:,1)); % Assumes first gt frame is 1 //Fgt指最大帧数
Ngt = length(unique(gtMat(:,2)));  %gt最多目标数
F = max(resMat(:,1)); %res最多目标数
missed=sum(m); %漏检率
falsepositives=sum(fp);  %虚警率
idswitches=sum(mme); %跳变率
